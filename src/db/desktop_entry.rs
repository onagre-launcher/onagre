...
use crate::db::{icon_to_str, Database, Entity};
...
impl<'a> DesktopEntryEntity<'a> {
    pub fn persist(entry: &dyn Entry, desktop_entry: &'a DesktopEntry, path: &Path, db: &Database) {
        let weight = match db.get_by_key::<DesktopEntryEntity>(DEFAULT_PLUGIN, &desktop_entry.name) {
    ...
    pub fn persist_with_mode(entry: &dyn Entry, mode: &str, db: &Database) {
        let weight = match db.get_by_key::<DesktopEntryEntity>(mode, &entry.get_display_name()) {
            Some(de_entry) => de_entry.weight + 1,
            None => 0,
        };
        ...
        let entity = DesktopEntryEntity {
            name: Cow::Borrowed(entry.get_display_name().as_ref()),
            icon,
            category_icon,
